...
class LandingComprarVenderController extends Controller
{
    public function indexEs($idCrypto = null, $idDivisa = null, $metodo = null)
    {
        ...
        return $this->CompraVenta($idCrypto, $idDivisa,$metodo, 'en');
    }
    ...
    public function CompraVenta($idCrypto = null,  $idDivisa = null, $metodo = null,$idioma)
    {
        try {

            if($metodo == null)
            {
                $data = PaymentMethods::where('id',1)->first();
            }else{
            ...
            if ($getConversNew) {
                ...
                $meta['title'] = __('index.titleCompraVende', ['cripto' => $getCriptodefault->name, 'currency' => $getCurrencyUser->name]);
                $meta['descripcion'] = __('index.descriptionCompraVende', ['currency' => $getCurrencyUser->name]);
                ...
                return view('landing.comprarvender', compact(
                    'data',
                    'meta',
                    'getCriptodefault',
                    'getCurrencyUser',
