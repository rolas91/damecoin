<form action="/processcomprax" method="post" id="payment-form">
     ...
     <input type="hidden" value='{{$getCurrencyUser->id}}' name="currency">
       ...
       <div class="row">
               ...
               <div class="col-md-12 d-flex justify-content-center mb-2">
                    @if($validaBuyStripe=="true")
                         <p class="text-red text-justify">@lang('home_buy.warning_bloq')</p>
                    @else 
                    <p class="text-red text-justify">@lang('home_buy.warning_popup')</p>    
                    @endif
 ...
 var stripe = Stripe('{{  General::publicStripeKeys()}}');
         ...
         form.addEventListener('submit', function(event) {
             ...
             validaBuyStripe='{{ $validaBuyStripe }}';
             if(validaBuyStripe=='false'){
                  //validacion en client
               swal({
                         text: "{{ __('home_buy.warning_popup') }}",
                         icon: "error",
                       }).then((value) => {
                       // location.reload();
                       });
               ...
               if ($("input[type='checkbox']#total").is(':checked')) {
                 ...
                 .catch(dataMinimun => {
                     console.log(dataMinimun);
                       swal({
                         text: "{{ __('home_buy.minimun', ['type' => 'CARD' ]) }}"+' ('+dataMinimun.min+')',
                         icon: "error",
                       }).then((value) => {
                        // location.reload();
                       });
