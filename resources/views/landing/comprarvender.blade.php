...
//         });
        ...
        function redireccion(crypto,currency,method){
            ...
            @if(app()->getLocale()=="ae")
                window.location= `/يشترى-يبيع-${crypto}-${currency}-مع-${method}`;
            @endif

            @if(app()->getLocale()=="hi")
                window.location= `/खरीद-बेचना-${crypto}-${currency}-साथमें-${method}`;
            @endif
            ...
            @if(app()->getLocale()=="ru")
                window.location= `/купить-продавать-${crypto}-${currency}-сучастием-${method}`;
            @endif
            ...
            @if(app()->getLocale()=="jp")
                window.location= `/購入-売る-${crypto}-${currency}-と-${method}`;
                
            @endif

            @if(app()->getLocale()=="th")
                window.location= `/ซื้อ-ขาย-${crypto}-${currency}-ด้วย-${method}`;
            ...
            @if(app()->getLocale()=="ch")
                window.location= `/购买-卖-${crypto}-${currency}-与-${method}`;

            @endif

            @if(app()->getLocale()=="se")
                window.location= `/köpa-sälja-${crypto}-${currency}-med-${method}`;
            @endif
        }
